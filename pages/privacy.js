        ...
        <div className="pure-page container mt-5">
          <div className="section-body row g-4 justify-content-center m-2 p-2">
            <div className="col-lg-8">
              <h1 className="mb-3">{data.privacy.title}</h1>
              <RichText text={data.privacy.text} />
            </div>
          </div>
        </div>
      </main>
...
export async function getStaticProps({ preview = false }) {
  ...
  */
  ...
  const data = {
    ...
    privacy: await getPrivacy(preview, locale),
  }
...
export async function getPrivacy(preview, locale) {
  const fields = `
  title
  text {
      json
  ...
  `
  const data = (await api.getData(preview, "privacyPage", fields, locale)) ?? []
  return data
}